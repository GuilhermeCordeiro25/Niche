...
TIMEOUT_SEGUNDOS = 60  # Se o Janus ficar 1 minuto inteiro sem emitir pulso, ele mata
ARQUIVO_HEARTBEAT = "heartbeat.txt"
ARQUIVO_LOG_SISTEMA = "watchdog_historico.log"
NOME_SCRIPT_JANUS = "main.py"

def buscar_pid_janus():
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            ...
            if proc.info['name'] and 'python' in proc.info['name'].lower():
                if proc.info['cmdline'] and any(NOME_SCRIPT_JANUS in cmd for cmd in proc.info['cmdline']):
...
def registrar_incidente(motivo):
    """Deixa um bilhete para o Janus ler depois e saber que apagou"""
...
def executar_auto_healing(pid_travado, motivo):
    ...
    logging.info("Ressuscitando o Janus...")
    # Levanta uma nova janela independente com o script
    subprocess.Popen(f"start python {NOME_SCRIPT_JANUS}", shell=True)
...
def iniciar_monitoramento():
    ...
    while True:
        ...
        pid_janus = buscar_pid_janus()
        
        if not pid_janus:
            logging.error("O Janus não está rodando no momento.")
        ...
        try:
            ...
            if tempo_sem_resposta > TIMEOUT_SEGUNDOS:
                logging.error(f"O Janus está catatônico! Sem sinais há {tempo_sem_resposta:.1f} segundos.")
                executar_auto_healing(pid_janus, f"Timeout de {TIMEOUT_SEGUNDOS}s estourado (Possível travamento de Driver/API)")
            else:
